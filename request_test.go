...
func TestNewRequest_multiHeader(t *testing.T) {
	...
	assert.Equal(t, `application/json`, r.Header.Get("Content-Type"))
	assert.Equal(t, `bar`, r.Header.Get("X-Foo"))
	assert.Equal(t, []string{"apex1", "apex2"}, r.Header[http.CanonicalHeaderKey("X-APEX")])
	assert.Equal(t, []string{"apex-1", "apex-2"}, r.Header[http.CanonicalHeaderKey("X-APEX-2")])
}
